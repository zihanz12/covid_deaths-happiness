...
author: "fa21-prj-shiyuan8-sw20-zihanz12-zl32</br>Shiyuan Wang, Songyuan Wang, Zihan Zhou, Kimmy Liu"
...
First, we included libraries such as :data.table, tidyverse, ggrepel, sf, dplyr, ggplot2, etc. for data cleaning, data visuilazation;</br>
...
Then, we used Package(countrycode) to assign each country to according region, and add the column to data.table(happiness)</br>
...
```
```{r}
head(happiness,4) # Before assigning each country to according region
```
...
```
```{r}
head(happiness,4) # After assigning each country to according region
```

After that, we implemented 'dcast' method to create desired data.table, and change the columnnames to avoid "&" and blank space. For example:</br>
...
**First, we plot "Life Satisfaction in each region during 2005-2020":**</br>

```{r,warning=FALSE,echo=FALSE}
graph <- ggplot(happiness_1,aes(x=Year)) +  geom_line(aes( y= Sub_Saharan_Africa,colour= "Sub_Saharan_Africa")) +  
...
graph + geom_line(aes( y= East_Asia_and_Pacific,colour= "East_Asia_and_Pacific"))+
...
**Then we plot plot "Life Satisfaction in 2018" for each country:** </br>
...
**After that, we Create world map for "life satisfaction" in each country in year 2018:**</br>

First, we merge dataset "world" and dataset "h1", and transform the output(world_) from data.frame to sf using method 'st_as_sf';</br>
...
First, we only want to analyze the countries which have a happiness index. </br>
```{r,echo=FALSE}
...
Then we want to visualize the Covid-19 cases and deaths in the happiest and least happiest country in the world, and try to see whether there exists a specific pattern. </br>
